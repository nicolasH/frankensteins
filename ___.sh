...
if [ "$1" = "new" ];then
    echo " new [title] : Creates an blog post at blog/`date +%Y_%m_%d.%H%M`.[prettified title].md ."
...
if [ "$1" = "gen" ];then
    ...
    ls *.md|sort -r|head -n 1|sed 's#\(....\).\(..\).\(..\)\(.*\)#echo "<div class=\"post\"><div class=\"date\">\1\/\2\/\3</div>";multimarkdown \1\_\2\_\3\4;echo "</div>"#'|bash >> index.html
