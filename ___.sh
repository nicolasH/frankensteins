...
if [ "$1" = "init" ];then
    echo " init        : Creating the blog directory and copies all the 'templates' files (_files) there."
...
if [ "$1" = "clean" ];then
    echo " clean       : Removing the generated html files by removing the date directories"
    cd blog
...
if [ "$1" = "gen" ];then
    ...
    ls *.md| sed 's#\(.\{10\}\)\(.*\)\.md#echo \1 \1\2.html >> titles;head -n 1 TK\1\2.md >> titles;cat post-header >> \1\2.html;head -n 1 TK\1\2.md >> \1\2.html;cat post-body >> \1\2\.html;multimarkdown TK\1\2.md >> \1\2\.html;cat post-footer >> \1\2\.html;#' |sed 's#_#\/#g'|sed 's# TK\(....\)\/\(..\)\/\(..\)\.# \1_\2_\3.#g' | sed 's: \(....\/..\/..\)\.: \1\/:g'|bash
...
echo "usage : victor.sh init new [title] clean gen";cat victor.sh | grep "echo \" "  | bash
